...
ARCH_PUBMED <- file.path("abstract-psilocybTi-set (1).txt")
...
leer_pubmed <- function() {
  ...
  lineas <- readLines(ARCH_PUBMED, warn = FALSE, encoding = "UTF-8")
  if (length(lineas) == 0) return(NULL)

  # Extrae el bloque de título y DOI del texto PubMed raw por cada línea de cita
  # con `doi:`. El texto nuevo no viene en formato CSV; se usa el patrón de
  # cabecera del registro seguida del título hasta `Author information:`.
  idx <- grep("doi:", lineas, ignore.case = TRUE)
  idx <- idx[!grepl("Erratum in|Comment in", lineas[idx], ignore.case = TRUE)]
  if (length(idx) == 0) return(NULL)

  out <- lapply(idx, function(k) {
    doi <- norm_doi(sub(".*doi:\\s*", "", lineas[k]))
    year <- regmatches(lineas[k], regexpr("\\b(19|20)\\d{2}\\b", lineas[k]))
    j <- k + 1
    while (j <= length(lineas) && trimws(lineas[j]) == "") j <- j + 1
    tit <- character()
    while (j <= length(lineas)) {
      if (grepl("^Author information:", lineas[j])) break
      if (grepl("^(Erratum in|Comment in)", lineas[j])) { j <- j + 1; next }
      if (trimws(lineas[j]) != "") tit <- c(tit, trimws(lineas[j]))
      j <- j + 1
    }
    data.frame(source = "PUBMED",
               title = paste(tit, collapse = " "),
               abstract = NA_character_,
               doi = doi,
               year = if (length(year) > 0) year else NA_character_,
